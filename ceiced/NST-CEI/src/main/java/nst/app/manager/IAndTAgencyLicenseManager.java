...
@Component
public class IAndTAgencyLicenseManager extends CEIBaseManager<IAndTAgencyLicense> {

  @Autowired
  IAndTAgencyLicenseRepository repository;
  ...
  @Override
  public BaseRepository<IAndTAgencyLicense> getRepository() {
    return repository;
  }

  @Override
  public IAndTAgencyLicense submitForm(IAndTAgencyLicense model) {
    performSubmit(model.getForm(), helper.toNGDTO(model));
    sendEmailOnSubmit(model);
    return repository.save(model);
  }

  public List<IAndTAgencyLicense> getMyApps() {
  ...
  public AgencyLegalStatusDetails addAgencyLegalStatusDetails(Long id) {
    IAndTAgencyLicense form = findForm(id);
